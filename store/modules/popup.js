const popup = {
	...
	mutations: {
		SET_PROMOTION_VISIBLE(state, visible) {
			state.promotionVisible = visible
		},
		SET_DOWNLOAD_VISIBLE: (state, visible) => {
			console.log(visible)
			state.downloadVisible = visible
			console.log(state.downloadVisible)
		},
		triggerMethodA(state, payload) {
			console.log(payload)
			state.callMethodA = payload;
		},
		...
		setShowPromotion: (state, data) => {
			console.log(data)
			state.showPromotion = data
		},
