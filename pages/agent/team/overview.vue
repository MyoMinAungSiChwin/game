...
export default {
  ...
  methods: {
    ...
    async orderLogInfo() {
      try {
        console.log('Filters:', this.filters); // Log filters before request
        let { data, code } = await zlList(this.filters);
        if (code == 200) {
          this.dataList = data;
          console.log('✅ Received dataList:', this.dataList); // Show the result in full
        } else {
          console.warn('❌ Failed with code:', code);
        }
      } catch (error) {
        console.error('❌ Error fetching zlList:', error);
      } finally {
        uni.hideLoading();
      }
    },
