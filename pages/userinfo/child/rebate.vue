<template>
	<view class="rebate">
		<view class="table_th game_flex align_center" style="text-align: center;">
			...
			<view class="rows">之前余额</view>
			<view class="rows">余额</view>
		    <view class="rows">之后余额</view>
		</view>
		<view class="table_td game_flex align_center" style="text-align: center;" :style="item.str == '-' ? 'color:red' : ''"
		v-for="item in list"
		:key="item.id"
		>
			<view class="rows">{{item.memo}}</view>
			<view class="rows">{{item.before}}</view>
			<view class="rows">{{item.money}}</view>
			
			<view class="rows">{{item.after}}</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			// 获取返利记录
			async getGameRebate(){
				let {code, data} = await gameRebate(this.form)
				if(code == 200){
					console.log(data)
					this.list = data.data
					this.total = data.tal
