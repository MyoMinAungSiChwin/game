<template>
	...
	<view class="d_jogos">
		 <div v-if="showSplash" class="splash-screen">
		      <img src="@/static/splash.jpg" alt="Splash Screen" class="splash-image" />
		</div>
	...
	export default {
		...
		methods: {
			...
			checkAppUrl() {
				...
				// Hide banner if matches exact condition
      ...
      async enterGameBtn(row) {
        ...
        if (data.code == 200) {
          ...
          // window.location.href = data.data.login_url
			...
			//
			//             // Handle protocol-relative URL (e.g. //example.com)
			...
			// 	}
			//   // uni.navigateTo({
			//   //   url: `/pages/jogos/webview/index?src=${targetUrl}`
			//   // });
			...
			//     window.open(targetUrl, '_blank');
			//   } else {
			//     // Other links open inside the app
			//     uni.navigateTo({
			//       url: `/pages/jogos/webview/index?src=${encodeURIComponent(targetUrl)}`
			//     });
			//   }
			//   // uni.navigateTo({
			//   //   url: `/pages/jogos/webview/index?src=${targetUrl}`
				...
				if (window.location.href.includes('#/?isApp=1')) {
					...
					try {
						...
						setTimeout(() => {
							...
							if (w) w.location.href = targetUrl;
						}, 50);
					} catch (e) {
						// 最终回退方案
						window.location.href = targetUrl;
			...
			//   targetUrl = targetUrl.trim();
			//
			//   // Handle Telegram tg: link - convert to https://t.me/ instead
			//   if (targetUrl.startsWith('tg:resolve?domain=')) {
			//     const username = targetUrl.split('tg:resolve?domain=')[1];
...
.splash-screen {
  ...
  width: 100%;
  height: 100%;
  background-color: #fff; /* Optional: for fade effect before image loads */
  z-index: 9999;
}
...
.splash-image {
  width: 100%;
  height: 100%;
  object-fit: cover;  /* This makes the image fill fully without distortion */
}
