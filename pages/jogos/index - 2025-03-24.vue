							...
							<view class="d_j_list d_bgColor d_border22" v-for="(key,value) in gameList" :key="value"
								...
								<view class="d_jl_list game_flex">
									<view class="d_jl_list_item" v-for="(item,child) in key.data" :key="item.id"
										...
										<view style="color: #fff; text-align: center; margin-top: 20rpx;">{{item.gameName}}</view>
									</view>
								</view>
							</view>
							<view class="d_flex" style="color: #fff; height: 344px;
					background-color: #181f2b;    align-items: center;" v-else>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleStart() {
				window.location.href = "https://jk.jingkaikj.com/index.html?token=" + this.userInfo.token;
				// this.$modal.msg("开通中...")
			},
			handleJoin() {
