<template>
  <view class="voucher_center">
    ...
    <view class="navHeader" style="margin-bottom:100rpx;">
      <u-navbar leftText="返回" bgColor="black" :height="50" :safeAreaInsetTop="false" @leftClick="leftClick">
        ...
        <view class="u-nav-slot" slot="center">
          <text style="font-size: 35rpx; color: #be984c;">{{$t('tzsCustomize.title36')}}</text>
        </view>
        <view class="u-nav-slot" slot="right">
          <view class="icon-container" @tap="goToDepost" style="margin-right: 10rpx;">
            <u-icon name="plus-square-fill" color="#be984c" size="20"></u-icon>
          </view>
        </view>
      </u-navbar>
    </view>
            ...
            <view class="card-body">
              <view class="details">
                ...
                <view class="detail-item">
                  ...
                  <text class="value">{{ item.status == 1 ? item.moneys || '-' : '-' }}</text>
                </view>
    ...
    <!-- Pickers -->
    <u-picker :show="czShow" :columns="[bankTypes.map(item => item.label)]" @confirm="czConfirm" @cancel="czClose" />
    <u-picker :show="typeShow" :columns="[statusTypes.map(item => item.label)]" @confirm="typeConfirm" @cancel="typeClose" />
    <u-calendar :show="timeShow" :title="$t('tzjl.text10')" mode="range" :minDate="minDate" :maxDate="maxDate" @confirm="confirm" @close="close" />
...
export default {
  ...
  computed: {
    ...mapGetters(['userInfo']),
    // Only withdrawals: type 1,3,4
    filteredRechargeList() {
      return this.rechargeList.filter(item => {
        const isWithdraw = [1,3,4].includes(item.type)
        ...
        return isWithdraw && matchBank && matchStatus
      })
    }
  },
  data() {
  ...
  methods: {
    ...
    goToDepost() {
      uni.navigateTo({ url: '/pages/voucher-center/deposit' })
    },
    tabsClick(e) {
    ...
    setDateRangeByTab() {
      ...
      if (this.activeIndex === 0) {
        this.startTime = this.endTime = fd(today)
      } else if (this.activeIndex === 1) {
        const y = new Date(today)
        y.setDate(today.getDate() - 1)
        this.startTime = this.endTime = fd(y)
      } else {
        const w = new Date(today)
        w.setDate(today.getDate() - 6)
        this.startTime = fd(w)
        this.endTime = fd(today)
      }
      this.timeRange = `${this.startTime} ~ ${this.endTime}`
    },
    async orderLogList() { /* your old method, ignore */ },
    async orderLogInfo() {
      const payload = {
        page: this.page,
        limit: this.limit,
      ...
      console.groupCollapsed('🔍 Request', payload); console.groupEnd()
      const res = await getOrderLogListInfo(payload)
      if (res.code === 200) {
        this.rechargeList = res.data.info.data || []
        this.total = res.data.count || 0
      }
    },
    allCz() { this.czShow = true },
    czConfirm(e) {
      this.selectedBankType = this.bankTypes[e.indexs[0]]
      this.orderLogInfo()
      this.czShow = false
    },
    czClose() { this.czShow = false },
    allType() { this.typeShow = true },
    typeConfirm(e) {
      this.selectedStatus = this.statusTypes[e.indexs[0]]
      this.orderLogInfo()
      this.typeShow = false
    },
    typeClose() { this.typeShow = false },
    allTime() { this.timeShow = true },
    confirm(e) {
