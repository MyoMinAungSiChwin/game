			...
			<view class="content">
				...
				<view class="payment-section">
					...
					<view class="payment-group left-group">
						<view class="group-title">
							<image class="group-title-icon" src="/static/images/cz/yh.png" />转账充值
						</view>
					...
					<view class="payment-group right-group">
						<view class="group-title">
							<image class="group-title-icon" src="/static/images/cz/zz.png" />加密货币充值
						</view>
						<view class="crypto-methods">
							<view class="method-card crypto-card " :class="czmsIndex == index ? 'active' : ''" v-for="(item, index) in czmsList" v-if="index <= 2"  @tap="czms(item,index)">
								<image class="method-icon" :src="item.avatar" />
								<text class="method-name">{{item.bank_name}}</text>
							</view>
							<view class="method-card crypto-card" v-if="czmsList.length <= 1">
				...
				<view class="input-group">
					<view class="input-wrapper">
						<input placeholder="请输入汇款姓名" class="custom-input" v-model="name" placeholder-style="color: #999;" />
					</view>
					<view class="input-wrapper">
				...
				<!-- 确认按钮 -->
				<button :class="btnDisabled ? 'confirm_btnDisabled' : 'confirm-btn'"  :disabled="btnDisabled" @tap="nextOrder">确认充值</button>
			</view>
		</view>
		<!-- 底部品牌 -->
