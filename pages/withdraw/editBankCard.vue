<template>
	<view class="voucher_center">
		<view class="navHeader" style="margin-bottom:100rpx;">
			<u-navbar leftText="返回" bgColor="black" :height="50" :safeAreaInsetTop="false" @leftClick="leftClick">
				<view class="u-nav-slot" slot="left">
					<u-icon name="arrow-left" color="#be984c" size="25"></u-icon>
				</view>
				<view class="u-nav-slot" slot="center">
					<view>
						<text style="font-size: 35rpx; color: #be984c;font-weight: bold;">编辑银行卡</text>
					</view>
				</view>
			</u-navbar>
		</view>
		<view class="flex-container account-form bank-card-container">
			<view class="inputCon" style="margin-bottom:17.2px" @tap="bankTypeClick">
				...
				<view class="inputIcon icon-down">
					<u-icon class="svg" name="arrow-down" color="#fff" size="25"></u-icon>
				</view>
			</view>
		...
		<!-- <u-action-sheet :actions="bankType" :show="show" title="选择银行"  @close="actionClose" :defaultIndex="[1]"
			@select="actionSelect"></u-action-sheet> -->
				<u-picker :show="show" :columns="[bankType]" keyName="name"   @close="actionClose" @confirm="actionSelect" :defaultIndex="[defaultIndex]"></u-picker>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.user_bank_id = option.id
			this.getDetailCardInfo()
			this.getBankTypeInfo()
		},
		mounted() {},
		beforeDestroy() {},
		methods: {
			async getDetailCardInfo() {
				let {
					data,
					code
				} = await detailCardInfo({id:this.user_bank_id})
				if (code == 200) {
					this.editData = data
					this.bank_number = data.bank_number
					this.bankTypeName = data.bank_type_name
					this.bank_type_id = data.bank_type_id
			...
			actionSelect(e) {
				this.bankTypeName = e.value[0].name
				this.bank_type_id = e.value[0].id
				this.bank_number = ''
				this.actionClose()
			},
