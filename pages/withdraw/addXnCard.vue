<template>
	<view class="voucher_center">
		<view class="navHeader" style="margin-bottom:100rpx;">
			<u-navbar leftText="返回" bgColor="black" :height="50" :safeAreaInsetTop="false" @leftClick="leftClick">
				...
				<view class="u-nav-slot" slot="center">
					<view>
						<text style="font-size: 35rpx; color: #be984c;font-weight: bold;">{{$t('account.text73')}}</text>
					</view>
				</view>
			</u-navbar>
		</view>
		<view class="flex-container account-form bank-card-container">
			<view class="inputCon" style="margin-bottom:17.2px" @tap="bankTypeClick">
				<view class="inputSelect">
					...
					<u-input type="text" class="pwd" :placeholder="$t('account.text74')"
			...
			<view class="account-form-group">
				<view class="mc-account-list">
					<u-input type="text" class="pwd" :placeholder="$t('account.text76')" name="withdraw"
			...
			<!-- 			<span class="am-button" style="margin-top: 50rpx;background-color: #2462c5; color: #fff;"
				@tap="$u.debounce(submitClick, 1500)">{{$t('account.text49')}}</span> -->
		...
		<u-action-sheet :actions="bankType" :show="show" :title="$t('account.text74')" @close="actionClose"
			...
			async submitClick() {
				...
				try {
					...
					if (uni.$u.test.isEmpty(this.bank_type_id)) {
						this.$modal.msg('请选择USDT类型')
						return
					}
					if (uni.$u.test.isEmpty(this.xn_address)) {
						this.$modal.msg('请填写USDT地址')
						return
					}
					if (this.hasChinese(this.xn_address)) {
						this.$modal.msg('USDT地址不能包含中文')
						return
					}
