<template>
	<view class="voucher_center">
		<view class="navHeader" style="margin-bottom:100rpx;">
			<u-navbar leftText="返回" bgColor="black" :height="50" :safeAreaInsetTop="false" @leftClick="leftClick">
				...
				<view class="u-nav-slot" slot="center">
					<view>
						<text style="font-size:35rpx; color: #be984c;font-weight: bold;">编辑电子钱包</text>
					</view>
				</view>
			</u-navbar>
		</view>
		<view class="flex-container account-form bank-card-container">
			...
			<!-- 	<view class="banks-wrapper">
					...
					<view class="bank-list-wrap">
						<view class="bank-list">
							<view class="bank-item has-icon" :class="usdt_group_index == index ? 'bank-active' : ''"
								@tap="groupClick(item,index)" v-for="(item,index) in groupData" :key="index">
			...
			<view class="account-form-group" style="margin-bottom: 17.2px !important;" v-if="userInfo.is_tx_password == 0">
			...
			<span class="am-button" @tap="submitClick">{{$t('account.text49')}}</span>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.user_usdt_id = option.id
			this.getUsdtGroupListInfo()
			this.getUsdtTypeInfo()
			this.getDetailUsdtCardInfo()
		},
		...
		beforeDestroy() {},
		methods: {
			async getDetailUsdtCardInfo() {
				let {
					data,
					code
				} = await detailUsdtCard({
					id: this.user_usdt_id
				})
				if (code == 200) {
					this.editData = data
					this.usdt_group_id = data.usdt_group_id
					this.usdt_type_id = data.usdt_type_id
					// this.usdt_group_name = data.usdt_group_name
					this.usdt_type_name = data.usdt_type_name
					this.moenyAddrss = data.money_address
					this.usdt_group_index = this.groupData.findIndex(item => {
						console.log(item.id)
						return (item.id) === (data.usdt_group_id);
					});

					this.usdt_type_index = this.typeData.findIndex(item => {
						console.log(item.id)
						return (item.id) === (data.usdt_type_id);
					});
