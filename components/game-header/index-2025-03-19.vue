<template>
	<view class="game_header">
		<u-navbar bgColor="#1c1b36" height="80rpx">
			<view slot="left" class="head_left">
				<u-icon class="off_icon" name="./static/images/jogos/unfold.png" size="48rpx"
					@tap="$store.dispatch('setFlPopup', true)"
					></u-icon>
			...
			<view slot="right" class="head_right game_flex align_center">
				...
				<view v-if="token" class="d_balance d_border22 d_flex">
					<text class="balance_txt">R$ {{userInfo.money}}</text>
					...
					@tap="$store.dispatch('setUserPopup', true)"
					>
						<u-avatar 
							:src="userInfo.avatar" shape="circle" size="50rpx"></u-avatar>
				...
				<view class="not_login game_flex" v-else>
					<u-button 
					:text="$t('jogos.text24')" @tap="setLoginBox(1)" color="#4faff8"
						type="primary">
					</u-button>

					<u-button 
					:text="$t('jogos.text25')"
					 color="#4180ed" type="primary" size="large"
						@tap="setLoginBox(2)">
					</u-button>
				</view>
		...
		:show="showLanguage" :columns="columns"
		keyName="label"
		></u-picker>
	</view>
</template>
