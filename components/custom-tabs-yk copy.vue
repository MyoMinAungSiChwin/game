		...
		<scroll-view scroll-x class="scroll-container" :scroll-left="scrollLeft">
			...
			<view class="tabs-container">
				<view v-for="(item, index) in tabsList" :key="index" class="tab-item"
					:class="{ 'active': current === index }" @click="switchTab(item,index)">
					<!-- 图标 -->
				<!-- 	<image :src="current === index ? item.activeIcon : item.icon" class="tab-icon" /> -->
					<!-- 文字 -->

					
					<text class="tab-label" v-if="$store.state.user.language == 'burmese'">{{ item.burmese == 'ပင်မစာမျက်နှာ' ? 'အားလုံး' : item.burmese }}</text>
					<text class="tab-label" v-if="$store.state.user.language == 'zh'">{{ item.title  == '首页' ? '全部' : item.title}}</text>
					<text class="tab-label" v-if="$store.state.user.language == 'english'">{{ item.english  == 'Home' ? 'All' : item.english}}</text>
				</view>
			</view>
		</scroll-view>
	</view>
